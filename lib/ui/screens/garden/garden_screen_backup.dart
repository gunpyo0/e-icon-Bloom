...
class _GardenScreenState extends ConsumerState<GardenScreen> {
  ...
  Future<void> _initializeLeague() async {
    ...
    // 리그 멤버 백업 실행 (기존 사용자들을 리그에 추가)
    try {
      print('Backing up league members...');
      await EcoBackend.instance.backupLeagueMembers();
    } catch (e) {
      print('Failed to backup league members: $e');
    }
    
    // 자동 리그 참여 확인 (여러 번 시도)
    for (int i = 0; i < 3; i++) {
      try {
        print('League join attempt ${i + 1}...');
        await EcoBackend.instance.ensureUserInLeague();
        break; // 성공하면 루프 종료
      } catch (e) {
        print('League join attempt ${i + 1} failed: $e');
        if (i < 2) {
  ...
  Widget _buildRankingList(ScrollController scrollController) {
    ...
    // 디버그: _leagueRanking 데이터 확인
    print('=== RANKING DEBUG ===');
    print('_leagueRanking length: ${_leagueRanking.length}');
    for (int i = 0; i < _leagueRanking.length; i++) {
      final member = _leagueRanking[i];
      print('Member $i: displayName="${member['displayName']}", point=${member['point']}, id="${member['id']}"');
    }
    ...
    final validMembers = _leagueRanking.where((member) => 
        ...
        member['displayName'].toString() != 'null'
    ).toList();
    
    print('Valid members after filtering: ${validMembers.length}');
    print('=====================');
